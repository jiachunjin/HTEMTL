...
function [Z_set, L_set] = learnZL_GD(W_1, Z_set, L_set, X_train, y_train, hp, para_ZL)
...
for iter = 1 : para_ZL.iter_max
    ...
    for i = hp.num_layer: -1 : 1
        ...
        % 3. Sum the gradient with the regularization term
        GZ{i} = G1_Z + G2_Z + hp.gamma_set(i) * Z_set{i};
        if hp.withL
            GL{i} = G1_L + G2_L + hp.gamma_set(i) * L_set{i};
        end

        % 4. Gradient descent
        Z_set{i}  = Z_set{i} - para_ZL.stepsize * GZ{i};
        if hp.withL
            L_set{i}  = L_set{i} - para_ZL.stepsize * GL{i};
        end
