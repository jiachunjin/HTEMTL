...
function [obj] = obj_value_pHTEMTL(X_train, y_train, W_set, Z_set, L_set, hp)
obj = 0;
...
for k = 1 : hp.num_layer
    ...
    elseif strcmp(hp.LR_sur, 'F')
        obj = obj + hp.gamma_set(k) / 2 * (norm(Z_set{k}, 'fro') ^ 2 + norm(L_set{k}, 'fro') ^ 2);
    end
end
obj = obj + hp.eta / 2 * norm(W_set{1}, 'fro') ^ 2;
end